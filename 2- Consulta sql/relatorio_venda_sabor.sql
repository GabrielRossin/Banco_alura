SELECT
    tp.sabor ,TO_CHAR(nf.data_venda, 'YYYY'),SUM( inf.quantidade) Quantida_vendida
...
SELECT
    TO_CHAR(nf.data_venda, 'YYYY'),SUM( inf.quantidade) Quantida_vendida
FROM itens_notas_fiscais inf
INNER JOIN tabela_de_produtos tp
ON inf.codigo_do_produto = tp.codigo_do_produto
INNER JOIN notas_fiscais nf 
...
Select
venda_sabor.sabor , venda_sabor.ano, venda_sabor.quantida_vendida, 
...
(
    SELECT
    tp.sabor ,TO_CHAR(nf.data_venda, 'YYYY') ano,SUM( inf.quantidade) Quantida_vendida
    FROM itens_notas_fiscais inf
    INNER JOIN tabela_de_produtos tp
    ON inf.codigo_do_produto = tp.codigo_do_produto
    INNER JOIN notas_fiscais nf 
...
(
    SELECT
    tp.tamanho ,TO_CHAR(nf.data_venda, 'YYYY') ano,SUM( inf.quantidade) Quantida_vendida
    FROM itens_notas_fiscais inf
    INNER JOIN tabela_de_produtos tp
    ON inf.codigo_do_produto = tp.codigo_do_produto
    INNER JOIN notas_fiscais nf 
